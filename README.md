...
- **イベント個別ページ** — 全27,137件をJSなしの静的HTMLとしてprerender（+sitemap.xml・前後ナビ・関連リンク・収録されている特集へのリンク）
- **キーボード操作** — `↑↓` 前後のイベント / `+` `-` ズーム / `/` 検索。右下のズームゲージで現在レベル（概観〜日）と可動域を常時表示
- **収録データの明示** — ヘッダーに総件数と収録期間（`全27,137件 · 1829年1月1日 〜 2026年8月6日`）を常時表示。値はビルド時に `static/data/index.json` から焼き込むため、description/OGPにも実数が載りpop-in/CLSが起きない
...
- 収録イベント: **27,137件**（1829-01-01 〜 現在。「YYYY年」+「YYYY年の日本」の2シリーズ+特集キュレーションを収録）
...
- 画像付きイベント: 9,664件（Wikimedia Commonsサムネイル）
- 関連リンク付きイベント: 10,700件（全体の約39%。自動算出+手動指定）
