...
- **特集（テーマ別の読み物）** — 「アニメの歴史」「ブレイクダンスの歴史」「観葉植物の歴史」「AI・コンピューティングの歴史」を年代順に通読できる `/c/<slug>`（prerender・JSなし）。年表側は `?k=<slug>` でその特集だけに絞り込める
...
- **ハイブリッドキュレーション** — 自動生成27,000件+ `content/curated/` の手動キュレーション391件（要約リライト・SVG割当・関連リンク手動指定）+ `content/collections/` の特集4本103件（うち37件は年ページに載らないテーマ史を出典つきで新規執筆）
...
- 特集: **4本 / 103件**（アニメ25・AI/コンピューティング41・ブレイクダンス18・観葉植物19。うち37件は新規執筆）
