...
歴史ニュースを縦の無限スクロール年表で。1868（明治）〜現在の国内外27,000件超の出来事を、
...
- **オリジナルSVGピクトグラム106点** — トップ層の歴史ニュースに統一スタイルの描き起こしイラスト（currentColorでテーマ・カテゴリ色に自動追従）
...
- **イベント個別ページ** — 全27,014件をJSなしの静的HTMLとしてprerender（+sitemap.xml・前後ナビ・関連リンク）
- **キーボード操作** — `↑↓` 前後のイベント / `+` `-` ズーム / `/` 検索。右下のズームゲージで現在レベル（概観〜日）と可動域を常時表示
- **収録データの明示** — ヘッダーに総件数と収録期間（`全27,014件 · 1868年1月1日 〜 2026年7月10日`）を常時表示。値はビルド時に `static/data/index.json` から焼き込むため、description/OGPにも実数が載りpop-in/CLSが起きない
...
- **ハイブリッドキュレーション** — 自動生成27,000件+ `content/curated/` の手動キュレーション391件（要約リライト・SVG割当・関連リンク手動指定）
...
- テスト: 217件 / `src/lib`・`pipeline/lib` 純関数層 カバレッジ100%（CIゲート）
- 実ブラウザスモーク: 22シナリオ（CIでも本番同等CSPで実行。280〜1280pxの横あふれ検証を含む）（`node e2e/smoke.mjs`）
- npm audit: 0件
